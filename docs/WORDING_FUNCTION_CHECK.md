...
| 관리번호 | `assetNo` | ✅ |
| 시리얼번호 | `serialNo` | ✅ |
| **지역** | `region` (신청자/수령 관련 **지역** — 코드관리 시트 지역 목록) | ✅ |
| **배송지** | `deliveryPlace` (수령지 — 배송지관리 시트) | ✅ |
| 전화번호 / 업체명 / 비고 | `phone`, `company`, `remarks` | ✅ |
| 사진 첨부 | `photoBase64` → 사진URL | ✅ |

※ 지역 = 신청 건의 지역(소속 지역), 배송지 = 실제 수령지. 서로 다른 필드로 정상 매핑됨.
...
### 3.3 지역 드롭다운에 주문 상태가 나오던 문제 (반영 완료)
- **증상**: 신규 신청 페이지 "배송/연락처"의 **지역** 드롭다운에 접수중·발주진행·발주완료 등 **주문 상태**가 노출됨.
- **원인**: 코드관리 시트 또는 API 응답에 상태 값이 지역으로 들어가 있음.
- **조치**  
  - **프론트** (`NewRequest.jsx`): `codes.regions()` 결과에 상태 값(접수중, 발주진행 등)이 섞여 있으면 **기본 지역 목록**(서울, 부산, 대구, 인천, 광주, 대전, 기타)을 사용하도록 변경.  
  - **백엔드** (`localStorageService.js`): `getRegions()`에서 **지역명이 주문 상태 문자열인 행은 제외**하고 반환하도록 필터 추가.
