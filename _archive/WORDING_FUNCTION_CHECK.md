...
| 워딩 | 데이터/기능 | 일치 |
|------|-------------|------|
| 상태 필터 (전체/접수중/진행중/완료/취소) | `statusFilter`: '', '접수중', 'inProgress', '처리완료', '접수취소. '진행중'은 접수완료·발주완료(납기확인/납기미정) 묶음 | ✅ |
...
| 접수 완료 버튼 | 상태 → `접수완료` (접수중일 때만). **담당자 비고 프롬프트에서 취소 시 상태 변경 없음** | ✅ (수정 반영) |
| 발주완료 버튼 | 상태 → `발주완료(납기확인)` (접수완료일 때만) | ✅ |
...
### 3.1 버그 수정 (반영 완료)
- **접수 완료 → 취소 시 발주완료로 바뀌던 문제**
  - **원인**: 담당자 비고 `window.prompt`에서 취소(null)해도 `updateStatus`가 호출됨.
  - **조치**: 비고가 필요한 경우(접수완료·발주완료 등) 프롬프트에서 **취소(null) 시 `updateStatus` 호출하지 않고 return** 하도록 수정 (`AdminRequests.jsx`).
...
- **상태**  
  - 접수중, 접수완료, 발주완료(납기확인/납기미정), 처리완료, 접수취소 — 모두 `status` 필드와 일치.
...
2. **상태**가 나오는 화면: 라벨이 `status` 값(접수중, 접수완료 등) 또는 그 집계(진행중, 완료 등)와 맞는지 확인.
