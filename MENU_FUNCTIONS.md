...
##### 4. 지연 건 모니터링
- 상태가 "접수완료" 또는 "발주지연"이고 발주일로부터 3일 이상 경과한 건
- 지연 일수 표시
...
|------------|------------|------|
| 로그인 | `/login` | |
| 사용자 대시보드 | `/dashboard` | 통계 카드(전체/접수중/진행중/처리완료/완료율), 최근 5건, 빠른 액션(새 신청/내 신청/내 정보) |
| 신청 등록 | `/new-request` | |
| 내 신청 목록 | `/my-requests` | 상태·날짜 필터, 통계 카드, 수령 확인 |
| 내 정보 | `/my-info` | 기본 정보 표시, 비밀번호 변경 |
| 신청 상세 | `/request/:requestNo` | 수령 확인(사용자), 상태/담당자 비고 저장(관리자) |
| 관리자 대시보드 | `/admin` | 조회 기간, 통계 카드, 긴급/지연 테이블, 기준정보 링크 |
| 전체 신청 목록 | `/admin/requests` | 상태·지역·날짜 필터, 통계/기준정보 링크 |
| 통계 및 리포트 | `/admin/statistics` | 기간 선택, 통계 카드, 상태별/지역별 분포 |
| 기준정보 관리 | `/admin/master` | 사용자/배송지/Excel 마스터 탭 |

### 9.2 네비게이션

- **사용자**: 대시보드, 신규 신청, 내 신청, 내 정보
- **관리자**: 관리자 대시보드, 전체 신청, 통계 및 리포트, 기준정보 관리

---

**최종 업데이트**: 2026-01-27  
**버전**: v1.0  
**상태**: ✅ Complete (React 적용 완료)
